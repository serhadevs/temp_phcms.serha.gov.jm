    ...
    public function fetchApplications(Request $request)
    {
        ...
        if ($app_type_id == 1) {
            if ($clinic_mode == "onsite") {
                $start_last_name = $request->filter_lastname ? $sign_off_params['start_last_name'] : '';
                $end_last_name = $request->filter_lastname ? $sign_off_params['end_last_name'] : '';
                    ...
                    if (!SignOff::where('application_id', $item)->where('application_type_id', $request->data["appTypeId"])->first()) {
                        ...
                        if ($request->data["appTypeId"] == 1) {
                            if ($application->permit_type == "student") {
                                ...
                                if($application->permit_category_id == 8){
                                    $expiry_date = date_format(date_modify(date_create($exam_date->test_date), "+2 years"), "Y-m-d");
                                }else{
