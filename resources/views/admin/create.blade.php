...
@section('content')
    ...
    <div class="main">
        ...
        <div class="container-fluid mb-4">
            <div class="card">
                <h4 class = "card-header">Add A Exam Date</h4>
                <form action="{{ route('examdate.store') }}" method="post">
                            ...
                            <div class="col-sm-9">
                                <select class="form-select" name="exam_day">
                                    ...
                                    @foreach ($exam_days as $key => $exam_day)
                                        <option value="{{ $key }}">{{ $exam_day }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-9">
                                <select class="form-select" name="permit_category_id">
                                    ...
                                    @foreach ($permitcategories as $permitcategory)
                                        <option value="{{ $permitcategory->id }}">{{ $permitcategory->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="card-footer">
                        ...
                        <button type = "submit" name="submit" class="btn btn-success">Add Exam Date</button>
                    </div>
                </form>
