...
@section('content')
    ...
    <div class="main">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('food_handlers_permit.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-3">
                            ...
                            <select id="" class="form-select" name="permit_category_id" {{-- onchange="populateSchedule(this.value)"> --}}
                                onchange="fetchPopulateExamSites(this.value)">
                        ...
                        <div class="row mt-3">
                            <div class="col">
                                ...
                                <select name="exam_sites" id="" class="form-select"
                                    onchange="fetchPopulateDates(this.value)">
                                    <option selected disabled>Please select exam site</option>
                                </select>
                            </div>
                            <div class="col">
                                <label for="" class="form-label">Exam Date</label>
                                <input type="text" class="form-control" id="exam_date_picker">
                            </div>
                            <div class="col">
                                <label for="" class="form-label">Exam Time</label>
                                <select name="exam_time" id="" class="form-select">
                                    <option selected disabled>Please select exam time</option>
                                </select>
                            </div>
                        </div>
                        <div class="mt-3">
        ...
        <script>
            //function to fetch and populate exam sites
            function fetchPopulateExamSites(permit_category_id) {
                $.post({!! json_encode(url('/exam/sites/permit-cat')) !!}, {
                    _method: "POST",
                    data: {
                    ...
                    if (data.status == 200) {
                        if (document.querySelector('select[name="exam_sites"]').childElementCount > 1) {
                            default_option = document.querySelector('select[name="exam_sites"]').children.item(0);
                            document.querySelector('select[name="exam_sites"]').innerHTML = "";
                            document.querySelector('select[name="exam_sites"]').appendChild(default_option);
                        }
                        data.exam_sites.forEach((exam_site) => {
                            document.querySelector('select[name="exam_sites"]').appendChild(createNewElement(
                                'option', [], {
                                    value: exam_site.id
                                }, exam_site.name));
                        });
            ...
            function fetchPopulateDates(exam_site_id) {
                if (!document.querySelector('select[name=permit_category_id]').value) {
                    swal.fire(
                        "Action Required",
                        "You need to select a permit category",
                        "error"
                    )
                } else {
                    $.post({!! json_encode(url('/booked/dates')) !!}, {
                        _method: "POST",
                        data: {
                            permit_category_id: document.querySelector('select[name=permit_category_id]').value,
                        ...
                        if (data.status == 200) {
                            console.log(data);
                            unavailableDates = data.excluded_dates;
                        } else {
                            swal.fire(
                                "Oops! Something went wrong.",
                                data.error,
            ...
            //function to fetch and populate time
            function fetchPopulateTime(appointment_date) {
                // console.log(document.querySelector('select[name="exam_time"]').value);
                $.post({!! json_encode(url('/filter/exam/times')) !!}, {
                    _method: "POST",
                    data: {
                        permit_category_id: document.querySelector('select[name=permit_category_id]').value,
                        exam_site_id: document.querySelector('select[name="exam_sites"]').value,
                        appointment_date: appointment_date
                    },
                    _token: "{{ csrf_token() }}"
                }).then(function(data) {
                    if (data.status == 200) {
                        if (document.querySelector('select[name="exam_time"]').childElementCount > 1) {
                            default_option = document.querySelector('select[name="exam_time"]').children.item(0);
                            document.querySelector('select[name="exam_time"]').innerHTML = "";
                            document.querySelector('select[name="exam_time"]').appendChild(default_option);
                        }
                        data.available_times.forEach((a_time) => {
                            document.querySelector('select[name="exam_time"]').appendChild(createNewElement(
                                'option', [], {
                                    value: a_time.exam_start_time
                                }, a_time.exam_start_time))
                        });
                    } else {
                        swal.fire(
                            "Oops! Something went wrong.",
            ...
            function createNewElement(type, class_list, attributes, inner_html) {
        ...
        <script>
            ...
            function unavailable(date) {
                ...
                if ($.inArray(ymd, unavailableDates) == -1 && date.getDay() != 0 && date.getDay() != 6) {
                    return [true, ""];
                } else {
                    return [false, "", "Unavailable"];
                }
            }
            $("#exam_date_picker").datepicker({
                dateFormat: 'yy-mm-dd',
                beforeShowDay: unavailable,
                onSelect: function(date) {
                    if (date != "") {
                        fetchPopulateTime(date);
                    }
                }
            });
